...
$stmt = $pdo->prepare("SELECT id, full_name, driving_school, phone, address, id_card_image FROM users WHERE role = 'instructor' AND status = 'active'");
$stmt->execute();
...
<body>
    ...
    <div class="instructor-container">
            <div class="mb-3">
                <label for="instructor_id" class="form-label">Search Instructor</label>
                ...
                <div class="instructor-card" data-id="<?= $instructor['id'] ?>">
                    <div class="instructor-info">
                        <div class="id-card-image">
                            <a href="/driving_school/uploads/id_cards/<?= htmlspecialchars($instructor['id_card_image']) ?>"
                            class="zoomable-image"
                            data-lightbox="instructor-<?= $instructor['id'] ?>"
                            data-title="<?= htmlspecialchars($instructor['full_name']) ?>">
                                <img src="/driving_school/uploads/id_cards/<?= htmlspecialchars($instructor['id_card_image']) ?>"
                                    alt="<?= htmlspecialchars($instructor['full_name']) ?>"
                                    class="instructor-image">
                            </a>
                        </div>
                        <div class="instructor-details">
                            <h3><?= htmlspecialchars($instructor['full_name']) ?></h3>
                            <p><strong>Driving School:</strong> <?= htmlspecialchars($instructor['driving_school']) ?></p>
                            <p><strong>Phone:</strong> <?= htmlspecialchars($instructor['phone']) ?></p>
                            <p><strong>Address:</strong> <?= htmlspecialchars($instructor['address']) ?></p>
                        </div>

                        <div id="calendar-<?= $instructor['id'] ?>" class="instructor-calendar"></div>
                        <button onclick="showBookingForm(<?= $instructor['id'] ?>, 'instructor')" class="btn btn-primary">
                            Show all available schedules
                        </button>
                    </div>
                </div>  
            <?php endforeach; ?>
        </div> <!-- ✅ Now the foreach is fully inside instructors-list -->
    </div>
    ...
    <div id="bookingModal" class="modal">
        <div class="modal-content">
            ...
            <form id="bookingForm" method="post" action="book.php">
                <input type="hidden" id="modal_instructor_id" name="instructor_id">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            var calendarEl = document.getElementById('calendar-<?= $instructor['id'] ?>');
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventClick: function(info) {
                    ...
                    showAvailableTimes(<?= $instructor['id'] ?>, dateStr);
                },
        ...
        function showAvailableTimes(instructorId, dateStr) {
            fetch('../includes/get_timeslots.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: `instructor_id=${instructorId}&date=${dateStr}&role=instructor`
            })
                ...
                .catch(err => {
                    ...
                    alert("Could not load timeslots. Please try again.");
                });
        }
       ...
       function showBookingForm(userId, role) {
            ...
            fetch(`/driving_school/includes/fetch_availability_list.php?user_id=${userId}&role=${role}`)
                ...
                .then(data => {
                    ...
                    const bookingAction = role === 'instructor'
