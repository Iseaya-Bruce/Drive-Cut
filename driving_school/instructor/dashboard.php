...
$stmt = $pdo->prepare("
    ...
    FROM ratings 
    WHERE rated_user_id = ? AND role = 'instructor'
");
$stmt->execute([$_SESSION['user_id']]);
$rating = $stmt->fetch();
...
<head>
    ...
    .instructor-dashboard .alert-success {
    ...
    .instructor-dashboard .upload-container {
        background: #f9f9f9;
    ...
    .instructor-dashboard .upload-container form {
        text-align: center;
    }

    .instructor-dashboard .current-image-wrapper img {
        max-width: 180px;
        border-radius: 10px;
    ...
    .instructor-dashboard .file-input-wrapper {
        margin: 20px 0;
    }

    .instructor-dashboard .file-button {
        background: #007bff;
        color: white;
        padding: 10px 15px;
        border-radius: 20px;
    ...
    .instructor-dashboard .file-name {
        margin-left: 10px;
        font-size: 0.9rem;
        color: #333;
    }

    .instructor-dashboard .upload-button {
        margin-top: 10px;
        background: #007bff;
        color: white;
        border: none;
    ...
    .instructor-dashboard .upload-button:hover {
        background: #0056b3;
    }

    .instructor-dashboard .upload-requirements {
        font-size: 0.85rem;
        color: #777;
        margin-top: 10px;
    }
    ...
    .instructor-dashboard th, .instructor-dashboard td {
        padding: 12px 10px;
        text-align: left;
        word-break: break-word;
    }

    .instructor-dashboard thead {
        background-color: #007bff;
        color: white;
    }

    .instructor-dashboard tbody tr:nth-child(even) {
        background-color: #f8f9fa;
    }

    .instructor-dashboard .btn-danger {
    ...
    .instructor-dashboard .btn-danger:hover {
        background-color: #c82333;
    }

    .instructor-dashboard .btn-sm {
        font-size: 0.85rem;
        padding: 6px 12px;
        border-radius: 20px;
        margin: 0 5px;
    ...
    .instructor-dashboard .btn-primary {
        background-color: #007bff;
        color: white;
        padding: 6px 14px;
        border-radius: 20px;
    ...
    .instructor-dashboard .btn-primary:hover {
        background-color: #0056b3;
    }

    .instructor-dashboard p a {
        color: #007bff;
    }

    .instructor-dashboard .action-buttons {
        margin-top: 20px;
    }
    ...
    @media (max-width: 768px) {
        ...
        .instructor-dashboard table, 
        .instructor-dashboard th, 
        .instructor-dashboard td {
            font-size: 0.95rem;
        }
    }
    ...
    @media (max-width: 480px) {
        ...
        .instructor-dashboard .btn-danger, .instructor-dashboard .btn-primary {
            width: 100%;
            display: block;
            margin-top: 5px;
        }
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if (!empty($booking['id_card_image'])): ?>
                                            <a href="../uploads/id_cards/<?= htmlspecialchars($booking['id_card_image']) ?>" 
                                            class="zoomable-image" 
                                            data-lightbox="id-card-<?= $booking['id'] ?>" 
                                            data-title="ID Card of <?= htmlspecialchars($booking['student_name']) ?>">
                                                <img src="../uploads/id_cards/<?= htmlspecialchars($booking['id_card_image']) ?>" 
                                                    alt="Student ID Card" 
                                                    class="thumbnail-id-card">
                                            </a>
                                        <?php else: ?>
