...
// Fetch ratings for the logged-in barber
$stmt = $pdo->prepare("SELECT r.*, u.full_name as student_name FROM ratings r JOIN users u ON r.student_id = u.id WHERE r.rated_user_id = ? AND r.role = 'barber' ORDER BY r.created_at DESC");
...
FROM ratings WHERE rated_user_id = ? AND role = 'barber'");
$stmt->execute([$_SESSION['user_id']]);
