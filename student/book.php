...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($slot && $slot['status'] === 'available') {
        ...
        if ($hoursDifference < 24) {
            ...
            header("Location: ../student/instructors.php");
            exit();
        }
        ...
        try {
            ...
            // Create booking
            $stmt = $pdo->prepare("INSERT INTO bookings (student_id, instructor_id, availability_id) VALUES (?, ?, ?)");
           ...
           // Open WhatsApp in new tab + redirect to dashboard
            echo "
            <script>
                window.open('$whatsapp_url', '_blank'); // open WhatsApp in a new tab
                window.location.href = 'dashboard.php'; // redirect current tab to dashboard
            </script>
            ";
            exit();
